...
%frameConnect connects some localizations if they are from one emitter in
%raw data. This helps to have a higher precision in localization at the end.
%
% INPUTS:
...
%                   connected (Pixels, Default=4)
%   MaxFrameGap:    Maximum Frame Gap between two localizations for being
%                   connected (Frame, Default=1)
%   FitType:        % Gaussian2D, Gaussian3D, SinglePSF, DoublePSF,
%                   (Default='Gaussian2D')
%
% OUTPUTS:
%   SMD:            Single Molecule Data, this function creats one more field
%                   to this structure: SMD.ConnectID
%   SMD_combined:   Single Molecule Data of connected localization.
%                   It includes only these fileds (X,Y,X_SE,Y_SE,N_Combined,
%                   FrameNum,Photons,Bg)
%
% REQUIRES:
%   Statistics Toolbox
%   mex file of c_code: c_FrameConnect in mex folder
%
% CITATION:
...
%Label the localization based on frame Connection function
%0 if they didn't pass Threshold and ~=0 if they passed. Its values
%are integer number for each cluster of localizations combined as
%a higher precesion.
